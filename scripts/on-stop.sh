#!/usr/bin/env bash
# Stop hook: fires when Claude finishes responding or when AGY terminates loop.
# Refreshes the auto-state block of HANDOFF.md and appends a DELTA_TRACKING entry
# every turn. Pushes only when no external committer (obsidian-git) is running.
...
# In AGY, we must output JSON on stdout for the Stop hook. We will capture our
# own logic stdout/stderr to prevent breaking the JSON contract.
exec 3>&1
exec 1>/dev/null
...
if ! as_find_config "$START_DIR" >/dev/null 2>&1; then
  if [ "$AGENT_ENV" = "antigravity" ]; then
    ...
    if [ "$AGENT_ENV" = "antigravity" ]; then
        # In AGY, transcriptPath is at root of JSON input, same key
        TRANSCRIPT="$(printf '%s' "$INPUT" | python3 -c 'import json,sys; d=json.loads(sys.stdin.read()); print(d.get("transcriptPath", ""))' 2>/dev/null)"
    fi
...
PY
    ...
    # Rewrite the hook-owned auto-state block of HANDOFF, preserving narrative.
...
if txt is None or START not in txt:
    narrative=("# Handoff — where we left off\n\n"
        "_Cross-device session handoff. The narrative below is maintained by the agent "
